    ...
    private static void saveToDb(Response response, Pair p) {
        ...
        for(int i = 0; i < response.getListings().size();  i++) {
            ...
            if (rawAddress.contains(" in ")) {
                ...
                address.append(rawAddress, rawAddress.lastIndexOf(", ")+2, rawAddress.length()); //Zip
            } else {
                address.append(rawAddress, 0, rawAddress.lastIndexOf(", ")+2);
                address.append(p.getValue() + " ");
                address.append(rawAddress, rawAddress.lastIndexOf(", ")+2, rawAddress.length());
            }
            ...
            try {
                insertStmt = "INSERT INTO " +
                        "leads(property_id, street, zipcode, address, city, area, asking_price, property_type, bed, bath, construction, lot, url, " +
                        "is_new_construction, last_update, prop_status, list_date, photo, baths_half, baths_full, photo_count, lat, lon, is_new_listing) " +
                        "VALUES " +
                        String.format("('%s', '%s', '%s', '%s' , '%s', '%s' , %f , '%s', %f , %f , %f , %f , '%s', %b, '%s', '%s', '%s', '%s', %f, %f, %f, %f, %f, %b)",
                                l.getProperty_id(),
